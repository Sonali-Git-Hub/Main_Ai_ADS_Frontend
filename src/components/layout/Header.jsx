    ...
    <header className="h-16 border-b border-slate-200 dark:border-slate-800/80 bg-white/80 dark:bg-[#090d16]/90 backdrop-blur-md sticky top-0 z-30 px-2 sm:px-6 flex items-center justify-between gap-1 sm:gap-4 transition-colors w-full overflow-x-clip">
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 min-w-0 flex-1 sm:flex-initial">
        ...
        <button
          ...
          className="lg:hidden p-1.5 sm:p-2 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 transition-colors shrink-0"
        ...
        {canGoBack && (
          <button 
            onClick={goBack}
            className="flex items-center gap-1 px-2 py-1.5 sm:px-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100/80 dark:bg-slate-900/80 hover:bg-brand-500/10 text-slate-800 dark:text-slate-200 hover:text-brand-500 dark:hover:text-brand-400 font-bold text-xs transition-all shadow-xs group shrink-0"
        ...
        <div className="relative min-w-0 shrink" ref={workspaceRef}>
          <button 
            ...
            className="flex items-center gap-1.5 sm:gap-2.5 px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-brand-500/50 bg-slate-100/70 dark:bg-slate-900/80 transition-all text-left max-w-full"
          >
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg sm:rounded-xl bg-white border border-slate-200 dark:border-slate-700 flex items-center justify-center overflow-hidden shrink-0 shadow-xs p-0.5 sm:p-1">
          ...
          {/* Workspace Dropdown */}
          {showWorkspaceMenu && (
            <div className="absolute top-full left-0 mt-2 w-64 max-w-[calc(100vw-1.5rem)] glass-card bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 py-2 z-50 animate-in fade-in slide-in-from-top-2">
      ...
      <div className="flex items-center gap-1 sm:gap-2.5 shrink-0 ml-auto sm:ml-0">
        ...
        <button 
          ...
          className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl bg-gradient-to-r from-violet-600/20 via-brand-500/20 to-cyan-500/20 border border-brand-500/40 hover:border-brand-400 text-slate-800 dark:text-slate-100 transition-all shadow-glow shrink-0"
          title="Manage Credits & Billing"
        >
          <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-cyan-400 animate-pulse shrink-0" />
          <div className="text-left hidden md:block">
            <div className="text-[9px] uppercase font-bold text-cyan-400 leading-tight">{t('visualCredits', 'Visual Credits')}</div>
            <div className="text-xs font-extrabold text-brand-400">{credits.balance} Credits</div>
          </div>
          <span className="text-[9px] sm:text-[10px] bg-brand-500 text-white font-extrabold px-1.5 py-0.5 rounded-full shrink-0 flex items-center gap-0.5">
            <span className="md:hidden font-bold">{credits.balance} · </span>
            {t('topUp', '+Top Up')}
          </span>
        </button>
        ...
        <button 
          onClick={toggleTheme}
          className="p-1.5 sm:p-2 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 transition-all shrink-0"
          title="Toggle Theme"
        >
