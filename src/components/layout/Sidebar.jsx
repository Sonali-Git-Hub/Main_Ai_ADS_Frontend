...
export const Sidebar = () => {
  ...
  return (
    <aside className={`h-screen sticky top-0 bg-[#090d16] border-r border-slate-800/80 flex flex-col justify-between transition-all duration-300 z-40 ${collapsed ? 'w-20' : 'w-64'}`}>
      <div>
        {/* Logo Header */}
            ...
            {!collapsed && (
              <div>
                <div className="flex items-center gap-1">
                  <span className="font-bold text-white text-base tracking-wide">AI Ads™</span>
                  <span className="text-[10px] bg-brand-500/20 text-brand-400 font-bold px-1 rounded">v1.0</span>
                </div>
                <p className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold">UWO Ecosystem</p>
              </div>
            )}
          </div>
          <button 
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-medium transition-all group relative ${
                  isActive 
                    ? 'bg-gradient-to-r from-brand-600/30 to-purple-600/10 text-white border border-brand-500/40 shadow-glow font-semibold' 
                    : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/40'
                }`}
              >
                <Icon className={`w-4 h-4 flex-shrink-0 transition-transform group-hover:scale-110 ${isActive ? 'text-brand-400' : 'text-slate-400'}`} />
                {!collapsed && <span className="truncate">{m.label}</span>}
                {isActive && <div className="absolute right-2 w-1.5 h-1.5 rounded-full bg-brand-400 animate-pulse"></div>}
              </button>
            );
          })}
        </nav>
