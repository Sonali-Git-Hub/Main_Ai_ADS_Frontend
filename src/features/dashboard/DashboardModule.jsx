...
export const DashboardModule = () => {
  ...
  const stats = [
    ...
    },
    {
      ...
      sub: `${analytics?.posts?.approved || 0} approved for publishing`,
      icon: PenTool,
    ...
    },
    {
      ...
      sub: 'Governed by Brand DNA memory',
      icon: ShieldCheck,
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center p-8 text-center space-y-3">
            ...
            <div className="space-y-1 max-w-sm">
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400">Create a campaign or generate content to see item status here.</p>
            </div>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-800 text-slate-500 uppercase tracking-wider text-[10px]">
                  ...
                  <th className="pb-3 px-3">Fact Check</th>
                  <th className="pb-3 px-3">Status</th>
                  <th className="pb-3 px-3 text-right">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {approvalsQueue.slice(0, 5).map((item) => (
                  <tr key={item._id || item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="py-3 px-3">
                      <span className="font-bold text-slate-900 dark:text-slate-200 block">{item.title}</span>
                      <span className="text-[10px] text-slate-500 uppercase font-semibold">{item.type} {item.platform ? `(${item.platform})` : ''}</span>
                    </td>
                    <td className="py-3 px-3 text-slate-600 dark:text-slate-300 font-medium">{item.author || 'AI Engine'}</td>
                    <td className="py-3 px-3">
                      <span className="inline-flex items-center gap-1 text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full text-[10px] font-bold">
